...
########################################################################################################
  ...
  if(any(hardNinety$FullName == athlete)) {
    ...
    hardNinety_ylim_min <- min(hardNinety$Cumulative2, na.rm = TRUE)
    hardNinety_ylim_max <- max(hardNinety$Cumulative2, na.rm = TRUE)
    
    last_two_points <- tail(hardNinety_graph_data$Cumulative2, 2)
    ...
    hardNinety_graph <- hardNinety_graph_data %>% 
      ggplot(aes(x = Date , y = Cumulative2, color = "")) +
