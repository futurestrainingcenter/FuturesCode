...
trackmanData <- trackmanData %>% 
  ...
  filter(!is.na(TaggedPitchType) & Month == "April" & PitchSession == "Live")
...
for (pitcher in pitchers){
  ...
  player_profile <- clientData %>%
    ...
    select(Name, Age, `Pitching Training/Booking Level`, `Position (Baseball/Softball)`, Gpa, HT_WT, `High School`, `Graduating Class`, Attendance)
  ...
  SVR_plot <- ggplot() +
    geom_text(aes(x = 0.5, y = 0.5, label = paste(round(SVR_average, digits = 2))),
