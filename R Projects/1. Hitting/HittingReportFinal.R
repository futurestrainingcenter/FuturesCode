...
hittraxSession <- read.csv("/Users/watts/Documents/Futures Performance Center/Data/Hittrax Data/Session/SessionExport_2024-05-28-08-20-19_UTC.CSV") %>% 
  mutate(TS = as.Date(TS, format="%m/%d/%Y %I:%M:%S %p"),
         Month = month(TS, label = TRUE, abbr = FALSE))
hittraxPlays <- read.csv("/Users/watts/Documents/Futures Performance Center/Data/Hittrax Data/Plays/PlaysExport_2024-05-28-08-20-19_UTC.CSV") %>% 
  mutate(TS = as.Date(TS, format="%m/%d/%Y %I:%M:%S %p"),
         Month = month(TS, label = TRUE, abbr = FALSE))
...
hittraxData <- hittraxData %>%
  ...
  ungroup() %>%
  group_by(`Skill Level`, Gender, Date) %>%
  mutate(
...
# Function to get width of text in ggplot2
get_text_width <- function(text, size, family) {
  strwidth(text, units = "figure", cex = size / 25, family = family)
}
...
for (athlete in athletes){
  ...
  filteredHittrax <- hittraxData %>% 
    filter(Month == "May" & Year == "2024" & Name == athlete)
  
  # Fetch the group of the current player
  current_level <- unique(filteredHittrax$`Skill Level`)[1]
  ...
  player_profile <- clientData %>%
    filter(Name == athlete) %>%
    mutate(Attendance = NA, HT_WT = paste(Height, " / ", Weight), `Age/Level` = paste(Age, " / ", `Skill Level`)) %>% 
    select(Name, `Age/Level`, `Position (Baseball/Softball)`, Gpa, HT_WT, `High School`, `Graduating Class`, Attendance, `Instagram Name (Optional)`, `Hitting Objective`)
  ...
  percentiles <- c('MaxVelRank_CumMax', 'AvgVel Rank', 'MaxDistRank_CumMax', 'AvgDist Rank')
  ...
  for (i in 1:length(metrics)) {
    ...
    datatable <- rbind(datatable, data.frame(
      ...
      `Rank` = level_rank,
      `Perc.` = filteredHittrax[[percentiles[i]]][1],
      check.names = FALSE
    ))
  }
...
##################################################################################################################
  ...
  # Fetch the group of the current player
  current_level <- unique(filteredBlast$`Skill Level`)[1]
  ...
  filtered_depth <- cleaned_data %>%
    filter(UserName == athlete, !is.na(Outcome), Outcome != "Caught Stealing", Month == "May") %>%
    ...
    group_by(GroupID, ExitVelocityGroup) %>%
    summarise(
  ...
  colors <- c("#3d9be9", "#83bff1", "#b5d9f7", "white", "#ffa3a3", "#ff4c4c", "#ff0000")
  breaks <- seq(1, 7, length.out = length(colors))
  labels <- c("<40", "40-49", "50-59", "60-69", "70-79", "80-89", "90+")
  
  if (nrow(filtered_depth) == 1) {
    geom_point_option <- geom_point(aes(fill = ExitVelocityGroup), shape = 21, size = 4, alpha = 0.85)
    scale_fill_option <- scale_fill_manual(values = colors[1], name = "Exit Velocity")
  } else {
    geom_point_option <- geom_point(aes(fill = GroupID), shape = 21, size = 4, alpha = 0.885)
    scale_fill_option <- scale_fill_gradientn(colours = colors, breaks = breaks, labels = labels, name = "Exit Velocity:")
    
  }
  
  depth_plot <- filtered_depth %>%
  ...
  ggsave(depth_plot,file=paste0("Futures Reports Images/",athlete," - depthplot.png"), width=3,height=3.3,units="in", dpi = 150)
  ...
  spray_plot <- geom_baseball(league = "mlb", color_updates = list(
    ...
    geom_bin_2d(data = filtered_spray, aes(x = PE1, y = PE3, fill = ..count..), alpha = 0.75, show.legend = FALSE) +
  ...
  PitchingReport7 <- image_composite(PitchingReport6,cropped_image, offset= "+2035+850")
  
  depthAVG <- mean(filtered_depth$Mean_Intersect3, na.rm = TRUE)
  ...
  PitchingReport8 <- image_composite(PitchingReport7,pitchCharts4, offset= "+1475+1275")
  
  sprayChart_metrics <- hittraxSession %>%
    filter(UserName == athlete, !(LDP == 0 & FBP == 0 & GBP == 0), Month == "May") %>%
    select(LDP, FBP, GBP) %>%
    summarise(
