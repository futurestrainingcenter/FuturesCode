...
max_velo_per_athlete <- pitchingData %>%
  group_by(Name, Level, Month) %>%
  summarize(MaxRelSpeed = max(RelSpeed, na.rm = TRUE), 
            .groups = 'drop')

# Rank athletes within each Level based on their fastest pitch
ranking_velo <- max_velo_per_athlete %>%
  group_by(Level, Month) %>% 
  arrange(Month, Level, desc(MaxRelSpeed)) %>%
  mutate(Rank = row_number()) %>%
...
filtered_strengthData <- strengthData %>% 
  filter(Month == "May", Level %in% c("L1", "L2", "L3"))

ISOSQT_per_athlete <- filtered_strengthData %>%
  filter(`Exercise Type` == "ForceDeck: ISO Belt Squat") %>% 
  group_by(Name, Level) %>%
  summarize(`Max Peak Vertical Force` = max(`Peak Vertical Force [N]`, na.rm = TRUE), .groups = 'drop')
