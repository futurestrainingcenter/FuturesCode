...
for (athlete in athletes){
  ...
  weight_plot <- CMJdata %>%
    filter(Name == "Aiden Soto") %>% 
    ggplot(aes(x = Date, y = `Weight (lbs)`, color = "")) +
    ...
    theme(legend.position = "none",
          ...
          plot.title = element_text(hjust = 0.5, color = "black", size = 18, family = "Good Times")) +
  ...
  core_subset <- finalExercise_data$`Highest Max`[finalExercise_data$`Exercise Name` == "Straight Arm Trunk Rotation Max Isometric Test - Crane Scale"]
  ...
  deadlift_subset <- finalExercise_data$`Highest Max`[finalExercise_data$`Exercise Name` == "Trap Bar Deadlift"]
  max_deadlift <- if (all(is.na(deadlift_subset))) NA else max(deadlift_subset, na.rm = TRUE)
  squat_subset <- finalExercise_data$`Highest Max`[finalExercise_data$`Exercise Name` == "Barbell Back Squat"]
  ...
  bench_subset <- finalExercise_data$`Highest Max`[finalExercise_data$`Exercise Name` == "Barbell Bench Press"]
  max_bench <- if (all(is.na(bench_subset))) NA else max(bench_subset, na.rm = TRUE)
  pulldown_subset <- finalExercise_data$`Highest Max`[finalExercise_data$`Exercise Name` == "Cable Lat Pull Down (1)"]
